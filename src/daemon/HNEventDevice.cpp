...
#include "HNEventDevicePrivate.h"
...
void 
HNEventDevice::defineOptions( OptionSet& options )
{
...
void 
HNEventDevice::handleOption( const std::string& name, const std::string& value )
{
...
int 
HNEventDevice::main( const std::vector<std::string>& args )
{
...
HNEventDevice::initConfig()
{
    ...
    if( cfgFile.saveConfig( HNODE_TEST_DEVTYPE, m_instanceName, cfg ) != HNC_RESULT_SUCCESS )
    {
        ...
        return HNEVD_RESULT_FAILURE;
    }

    return HNEVD_RESULT_SUCCESS;
}

HNEVD_RESULT_T
HNEventDevice::readConfig()
{
    HNodeConfigFile cfgFile;
    HNodeConfig     cfg;
...
void 
HNEventDevice::dispatchEP( HNodeDevice *parent, HNOperationData *opData )
{
    std::cout << "HNEventDevice::dispatchEP() - entry" << std::endl;
